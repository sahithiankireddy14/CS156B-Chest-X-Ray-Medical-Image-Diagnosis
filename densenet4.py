...
PRINT_FREQ = 1000
EPOCHS = 50
OUTPUT_NAME = 'predicted_support.csv'
CHECKPOINT_NAME = 'model_support.h5'
COL = 15
...
testpath = []
testimg = []
with open('../../data/student_labels/test_ids.csv', newline='') as csvfile:
    spamreader = csv.reader(csvfile, delimiter=',', quotechar='|')
    line1 = True
    count = 0
    for row in spamreader:
...
with open(OUTPUT_NAME, 'w', newline='') as csvfile:
    ...
    for i in range(len(testid)):
        spamwriter.writerow([testid[i],testpath[i],-1,0,0,1,0,0,1,0,least_mse(predicted[i])])